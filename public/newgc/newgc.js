async function newgc(event) {
    ...
    await axios.post("http://52.72.228.99:1000/group/addnewgc", groupData, { headers: { "Authorization": localStorage.getItem("token") } })
        .then(response => { 
...
async function deleteuser (event) {
    ...
    console.log(userObj)
    if(userObj.groupid!= ""  &&  userObj.username!="" && userObj.phonenumber!="" ){
        const response = await axios.post("http://52.72.228.99:1000/group/deleteuser", userObj,{ headers: { "Authorization": localStorage.getItem("token") } })
        .then(response => response)
